 ...
 * It provides the necessary information for the Fabric loader to locate and launch the game.
 *
...
public class QuantumVxlGameProvider implements GameProvider {
    ...
    private Path commonGameJar;
    private Path kotlinJar;
    ...
    public boolean locateGame(FabricLauncher launcher, String[] args) {
        ...
        try {
            ...
            var clientLib = GameLibrary.QUANTUM_VXL_LWJGL3;
            var serverLib = GameLibrary.QUANTUM_VXL_COMMOn;

            // Get the common game jar and check if it is declared
            var lwjgl3Jar = GameProviderHelper.getCommonGameJar();
            var commonGameJarDeclared = lwjgl3Jar != null;

            // Process the common game jar if it is declared
            if (commonGameJarDeclared) {
                classifier.process(lwjgl3Jar);
            }
            ...
            lwjgl3Jar = classifier.getOrigin(GameLibrary.QUANTUM_VXL_LWJGL3);
            clientJar = classifier.getOrigin(GameLibrary.QUANTUM_VXL_CLIENT);
            commonGameJar = classifier.getOrigin(GameLibrary.QUANTUM_VXL_COMMOn);
            var serverJar = classifier.getOrigin(GameLibrary.QUANTUM_VXL_DEDICATED_SERVER);
            this.libGdxJar = classifier.getOrigin(GameLibrary.LIBGDX);
            this.kotlinJar = classifier.getOrigin(GameLibrary.KOTLIN);

            // Warn if the common game jar didn't contain any of the expected classes
            if (commonGameJarDeclared && lwjgl3Jar == null) {
            ...
            if (commonGameJar != null) {
                ...
                suppressedExceptions.add(new FormattedException("No common game jar found", "No common game jar found for Quantum Voxel"));
            }
            ...
            if (this.kotlinJar != null) {
                this.gameJars.add(this.kotlinJar);
            }
